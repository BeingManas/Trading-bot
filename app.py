...
def get_client():
    """Create a Binance client. Checks DEMO_MODE env var, then auto-detects."""
    api_key = os.getenv("BINANCE_API_KEY", "")
    api_secret = os.getenv("BINANCE_API_SECRET", "")
    # Check DEMO_MODE env var first; if not set, auto-detect from keys
    demo_env = os.getenv("DEMO_MODE", "").lower()
    if demo_env in ("true", "1", "yes"):
        demo = True
    elif demo_env in ("false", "0", "no"):
        demo = False
    else:
        demo = not (api_key and api_secret)
    if demo:
        logger.info("Running in DEMO mode (simulated orders with real prices)")
    else:
        logger.info("Running in LIVE TESTNET mode")
...
def get_ticker(symbol):
    """Get 24hr ticker stats for a symbol (price, change, volume)."""
    try:
        import requests as req
        r = req.get(
        ...
        if r.status_code == 200:
            ...
            return jsonify({
                ...
                "priceChangePercent": d.get("priceChangePercent"),
                "highPrice": d.get("highPrice"),
                "lowPrice": d.get("lowPrice"),
                "volume": d.get("volume"),
                "quoteVolume": d.get("quoteVolume"),
            })
        return jsonify({"success": False, "error": "Could not fetch ticker"})
    except Exception:
        return jsonify({"success": False, "error": "Ticker unavailable"})
...
def get_klines(symbol):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return jsonify({"success": True, "klines": klines})
        return jsonify({"success": False, "error": "Could not fetch klines"})
    except Exception:
        return jsonify({"success": False, "error": "Klines unavailable"})
...
def get_depth(symbol):
    ...
    try:
        ...
        return jsonify({"success": False, "error": "Could not fetch depth"})
    except Exception:
        return jsonify({"success": False, "error": "Depth unavailable"})
